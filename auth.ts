...
export const { handlers, auth, signIn, signOut, unstable_update } = NextAuth({
  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      ...
      // Fired by unstable_update({ user: { tenantId, refreshMemberships? } }) — swaps the active tenant without a re-login; refreshMemberships also re-queries the DB since a brand-new membership (e.g. from creating an org) isn't in t.memberships yet.
...
// Shared by authorize() and the jwt callback's refreshMemberships path (an Account gaining a new Tenant needs its token's membership list to catch up).
